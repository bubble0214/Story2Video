  ...
  return (
    <div className="max-w-5xl mx-auto py-8 px-4 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold tracking-tight">剧本</h1>
        <Button variant="outline" onClick={() => router.push('/assets/script')}>返回资产</Button>
      </div>
      <ScriptTab
        content={(result.script_content as string) || '未生成剧本内容。'}
        scenes={result.generated_scenes as { num: string; content: string; location?: string; summary?: string }[] | undefined}
      />
    </div>
  );
}
